(function() {
    var app = angular.module('olmap-directive', ['alert-handler','cgBusy','device-services']);

    /**
     * Directive to show devices in a map
     */
    app.directive('olmap', function($http,$state,$q,$timeout,Alert,Device) {
      return {
          restrict: 'A',
          scope: {
            folderId:'=',
		...
		scope.folderLoading.then( function(device) {
               
                if(typeof device.properties =='undefined' || typeof device.properties['mapUri'] =='undefined'){
                  Alert.open('warning','This folder does not have a map');
                  return;
                }
                scope.folder = device;
		scope.mapImg = device.properties['mapUri'];
	
                if(typeof scope.deviceId != 'undefined'){
                  scope.folderLoading = Device.construct(scope.deviceId,true).then(function(device){
                    scope.devices.push(device);
                    initMap();
                  });
                }else{
                      var deviceRequests = [];
		      if (typeof device.references != 'undefined' && typeof device.references.children != 'undefined') {
                      	for(var i in device.references.children){
                      	  var id = device.references.children[i].id;
                      	  var req = Device.constructDevice(id,true).then(function(response){
