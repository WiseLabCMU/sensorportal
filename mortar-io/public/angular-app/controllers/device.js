...
(function() {
	 ...
	 */
	app.controller('DeviceCtrl', function($scope, $state, $stateParams, User,
		Device, Alert) {
		$scope.folder = Device;
		$scope.user = User;
		$scope.device = null;
	 ...
	 * @return {[type]}                [description]
	 */
	app.controller('DevFavoritesModalCtrl', function($scope, $modal, $modalInstance, $state, $stateParams, User, Alert, Device, Browser, $q, $timeout) {
				...
				$q.all($scope.allPromise).then(function(response) {
					...
					for (var i = 0; i < response.length; i++) {
						if (typeof response[i].error != 'undefined') {
							...
							console.log("sadfklasdjfa;sjfksal;dkjf" + $scope.newFavorites[i].id);
