(function() {
  ...
  app.directive('timeseries', function($http, $timeout, Alert) {
      return {
          ...
          templateUrl: '/components/device/deviceTimeseries/timeseries/timeseries.html',
          link: function(scope, element, attrs) {
            ...
            var inDeviceTransducers = function(deviceid,transducer){
              if(deviceid != scope.device.id){
                return false;
              }
              for(var t in scope.device.transducers){
                if(t.split(' ').join('-') == transducer){
                  return true;
                }
              }
              return false;
	        ...
	        if (typeof scope.device == 'undefined' || typeof scope.device.storage == 'undefined' || 
		    scope.device.storage.length == 0) {
			//Alert.open("Storage item did not load");
			scope.gettingGraphs = false;
			scope.noGraphs = true;
			return;
		}
                var strDeviceId = scope.device.id;
                urlDomains[strDeviceId] = scope.device.storage;
                $.ajax({
                  url: urlDomains[strDeviceId][0].address+'/info.json',
			...
			if (typeof key == 'undefined') continue;
                        var device_name = key.split('.')[0];
                        var chan = key.split('.')[1];
                        if (inDeviceTransducers(device_name,chan)) {
                            addedGraph =true;
                            element.append(' \
                           <div id="'+device_name+chan+'"> \
                           ...
                           ');
                            ...
                            plots[i] = createPlot(device_name, chan, plot5Style, true, dateAxes[i], "yAxis" + (i + 1), {
