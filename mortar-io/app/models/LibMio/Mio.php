...
class Mio {
  ...
  private $toolsPath = '/Workspace/tmp/mortr.io/git/mio/tools/cmd-line/c/bin/';
  ...
  /**
   * Tries to authenticate an user against xmpp server
  ...
  public function auth($strUsername,$strPassword){
    ...
    return array('error'=>true,'message'=>'Could not log you in to '.$this->host);
  }
  /**
  ...
  */
  ...
  public function getUsers($strUsername,$strPassword){
    $strUsernamesTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl registered_users %s',$this->host);
    $strLastOutput = exec($strUsernamesTool,$arrOutput,$intReturnCode);
    $arrUsers = array();
    if($intReturnCode==0) {
      foreach($arrOutput as $username) {
        $strUserDataTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_get %s %s user http://mortar.io/user',$username,$this->host);
        $xmlOutput = '';
  ...
  public function displayUser($strUsername){
    $strUserDataTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_get %s %s user http://mortar.io/user',$strUsername,$this->host);
    $strLastOutput = exec($strUserDataTool,$xmlOutput,$intReturnCode);
    if($intReturnCode==0) {
      $xml = simplexml_load_string($xmlOutput[0]);
      $arrUser = array();
      if(isset($xml->name) && isset($xml->email) && isset($xml->group)){
        ...
        return array('error'=>true, 'message'=>'No user metadata.');
      }
    }
    return array('error'=>true,'message'=>'Error executing ejabberdtctl private_get.');
  }
  ...
  public function createUser($strUsername,$strPassword,$strName,$strEmail,$strGroup){
    ...
    $strCreateUserTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl register %s %s %s',$strUsername,$this->host,$strPassword);
    ...
    if($intReturnCode==0){
      ...
      $strAddMetaTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_set %s %s ',$strUsername,$this->host);
  ...
  public function editUser($strUsername,$strPassword=null,$strName,$strEmail,$strGroup){ 
    ...
    $strXmlData =  sprintf("<user xmlns='http://mortar.io/user'><name>%s</name><email>%s</email><group>%s</group></user>",$strName,$strEmail,$strGroup);
    $strEditMetaTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_set %s %s ',$strUsername,$this->host);
  ...
  public function setUserPassword($strUsername,$strPassword){
    $strChangePwdTool = sprintf('sudo /Applications/ejabberd-15.11/bin/ejabberdctl change_password %s %s %s',$strUsername,$this->host,$strPassword);
  ...
  public function deleteUser($strUsername){
    $strDeleteTool = 'sudo /Applications/ejabberd-15.11/bin/ejabberdctl unregister '.$strUsername.' '.$this->host;
    $strDeleteLastOutput = exec($strDeleteTool,$arrDeleteOutput,$intDeleteReturnCode);

    // Check if user account still exists
    $strCheckAccountTool = 'sudo /Applications/ejabberd-15.11/bin/ejabberdctl check_account '.$strUsername.' '.$this->host;
  ...
  public function actuate($strEvent,$strTransducer,$strValue,$strUsername,$strPassword){
    $strUsername .= '@'.$this->host;
    $strActuateTool = $this->toolsPath.'mio_actuate'. sprintf(' -event "%s_act" -id "%s" -value %s -u %s -p %s',$strEvent,$strTransducer,$strValue,$strUsername,$strPassword);
   ...
   * @return mixed array containing error
   */
  public function getTransducerLastValue($strEvent,$strTransducer,$strUsername,$strPassword){
    $strUsername .= '@'.$this->host;
    $strTransducerTool = $this->toolsPath. 'mio_item_query_stanza'. sprintf(' -event "%s" -item "%s" -u %s -p %s',$strEvent,$strTransducer,$strUsername,$strPassword);
    $strLastOutput = exec($strTransducerTool,$arrOutput,$intReturnCode);
    if($intReturnCode == 1 && !empty($arrOutput)){
      //$arrOutput = "<items>" . $arrOutput[0] . "</items>";
      return array('error'=>false,'data'=>json_decode(json_encode(simplexml_load_string(implode(" ",$arrOutput))),true));
    }
    return array('error'=>true,'message'=>'could not get meta data');
  }
  ...
  public function setRecoveryCode($strUsername,$strToken){
    ...
    $strRecoveryCreateTool = sprintf("sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_set %s %s ",$strUsername,$this->host);
  ...
  public function getRecoveryCode($strUsername){
    $strRecoveryGetTool = sprintf("sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_get %s %s %s '%s'",$strUsername,$this->host,'recovery','http://mortar.io/user_recovery');
  ...
  public function listMonitors($strUsername,$strPassword){
    $strListMonitorTool = sprintf("sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_get %s %s %s %s",$strUsername,$this->host,'monitors','http://mortar.io/monitors');
  ...
  public function saveListMonitors($arrMonitors,$strUsername,$strPassword){
    ...
    $strSLMonitorTool = sprintf("sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_set %s %s ",$strUsername,$this->host);
  ...
  public function saveMonitor($strId,$strMonitor,$strUsername,$strPassword){
    $strSaveMonitorTool = sprintf("sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_set %s %s ",$strUsername,$this->host);
  ...
  public function getMonitor($strNS,$strUsername,$strPassword){
    $strGetMonitorTool = sprintf("sudo /Applications/ejabberd-15.11/bin/ejabberdctl private_get %s %s %s '%s'",$strUsername,$this->host,'monitor',$strNS);
