...
class Folder {
  ...
  public function createFolder($strUsername,$strPassword){
    ...
    if(isset($this->isFavorite) && !empty($this->isFavorite) && !is_null($this->isFavorite)){
      $return=$this->addProperty('isFavorite',$this->isFavorite,$strUsername,$strPassword);
      if($return['error']){
        return array('error'=>true,'message'=>'Could not add the isFavorite property');
      }
    }
    return $return;
  }
