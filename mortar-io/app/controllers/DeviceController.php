...
class DeviceController extends BaseController {
  ...
  public function getTransducers($strDeviceId){
    try{
        $this->transducers = Device::getTransducers($strDeviceId,$this->user['username'],$this->user['password']);
      // $this->getTransducersValue($strDeviceId,$this->user['username'],$this->user['password']);
    }catch(Exception $e){
      return Response::json(array('error'=>true,'message'=>$e->getMessage()));
    }
    return Response::json(array('error'=>false,'transducers'=>$this->transducers));
  }
  /**
  ...
  public function getTransducersLastValue($strDeviceId){
    $device = new Device;
    $device->id = $strDeviceId;
    $response = $device->getTransducersLastValue($strDeviceId,$this->user['username'],$this->user['password']);
    return Response::json($response);
  }
}
